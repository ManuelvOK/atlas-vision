<rect width="900" height="600" bg="170" border="true" attributes="window">
    <rect width="580" height="530" x="20" y="20" border="true" attributes="player">
        <rect width="580" height="530" x="10" y="0" attributes="player-grid">
        </rect>
        <rect width="580" height="100" x="10" y="100" attributes="core-1">
            <rect width="570" height="20" x="0" y="10" bg="180" border="true" attributes="atlas scheduler">
            </rect>
            <rect width="570" height="20" x="0" y="40" bg="190" border="true" attributes="recovery scheduler">
            </rect>
            <rect width="570" height="20" x="0" y="70" bg="200" border="true" attributes="cfs scheduler">
            </rect>
        </rect>
        <rect width="580" height="530" x="10" y="0" attributes="player-position">
            <vertical-line color="red" height="530" x="0" attributes="player-position-line" />
        </rect>
        <rect width="580" height="100" x="0" y="0" attributes="deadline">
        </rect>
    </rect>
    <rect width="190" height="530" x="650" y="20" border="true" attributes="sidebar">
        <rect width="40" height="400" x="0" y="0" border="true" attributes="legend">
        </rect>
        <rect width="150" height="350" x="40" y="0" border="true" attributes="dependency">
        </rect>
        <rect width="190" height="170" x="0" y="400" border="true" attributes="messages">
        </rect>
    </rect>
</rect>
