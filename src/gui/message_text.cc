...
MessageText::MessageText(const std::string message, TTF_Font *font, int width, int offset_y) :
    ...
    this->_surface_active = TTF_RenderText_Blended_Wrapped(this->_font, message.c_str(), color, width);
    /* TODO: get color from config */
    color = {255, 255, 255, 180};
    this->_surface_inactive = TTF_RenderText_Blended_Wrapped(this->_font, message.c_str(), color, width);
