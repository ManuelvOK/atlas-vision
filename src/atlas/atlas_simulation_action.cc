...
void SubmissionAction::execute() {
    ...
    std::vector<AtlasJob *> jobs = this->_atlas_model->next_atlas_scheduled_jobs(core);
    ...
    for (auto job_it = jobs.rbegin(); job_it != jobs.rend(); ++job_it) {
        ...
        this->_atlas_model->_actions_to_do.push_back(
            new DeadlineAction(this->_atlas_model, s->atlas_job()->_deadline, s->atlas_job()));
    ...
    if (not this->_atlas_model->active_schedule_on_scheduler(core, AtlasSchedulerType::CFS,
                                                             this->_atlas_model->_timestamp)
        and not this->_atlas_model->active_schedule_on_scheduler(core, AtlasSchedulerType::ATLAS,
                                                                 this->_atlas_model->_timestamp)) {
...
void FillAction::execute() {
    ...
    if (not this->_atlas_model->_recovery_queue[this->_core].empty()) {
        ...
        AtlasJob *job = this->_atlas_model->_recovery_queue[this->_core].front();

        int time_left = job->estimated_execution_time_left(timestamp);
        BaseAtlasSchedule *next_atlas_schedule =
            this->_atlas_model->next_atlas_schedule(this->_core);
    ...
    /* Check if there is something in CFS queue */
        ...
        AtlasJob *job = this->_atlas_model->_cfs_queue[this->_core].front();
...
template<typename T>
void BeginScheduleAction<T>::end_schedule(BaseAtlasSchedule *schedule) {
    if (not schedule) {
        return;
    }
    int timestamp = this->_atlas_model->_timestamp;
...
template<>
void BeginScheduleAction<LateCfsSchedule>::execute() {
    int timestamp = this->_atlas_model->_timestamp;
    ...
    message << "Execute job " << this->_schedule->job()->_id << " on core "
            ...
            << time_left << " (" << time_left * this->_atlas_model->_cfs_factor << " on CFS)";
    this->_atlas_model->add_message(timestamp, message.str(), {this->_schedule->job()->_id});
...
template<>
void BeginScheduleAction<EarlyCfsSchedule>::execute() {
    int timestamp = this->_atlas_model->_timestamp;
...
template<>
void BeginScheduleAction<RecoverySchedule>::execute() {
    int timestamp = this->_atlas_model->_timestamp;
...
template<>
void BeginScheduleAction<AtlasSchedule>::execute() {
    int timestamp = this->_atlas_model->_timestamp;
...
template<>
void EndScheduleAction<EarlyCfsSchedule>::execute() {
    AtlasJob *job = this->_schedule->atlas_job();
    int timestamp = this->_atlas_model->_timestamp;
...
template<>
void EndScheduleAction<LateCfsSchedule>::execute() {
    /* check if execution is finished */
    if (this->_schedule->job()->execution_time_left(this->_atlas_model->_timestamp) <= 0) {
        /* there is place for a new schedule */
...
template<>
void EndScheduleAction<RecoverySchedule>::execute() {
    int timestamp = this->_atlas_model->_timestamp;
    AtlasJob *job = this->_schedule->atlas_job();
...
template<>
void EndScheduleAction<AtlasSchedule>::execute() {
    AtlasJob *job = this->_schedule->atlas_job();
    int timestamp = this->_atlas_model->_timestamp;
    ...
    if (time_left <= 0) {
        ...
        this->_atlas_model->_cfs_queue[this->_schedule->_core].push_back(this->_schedule->atlas_job());
    } else {
        std::stringstream message;
        ...
        this->_atlas_model->_recovery_queue[this->_schedule->_core].push_back(this->_schedule->atlas_job());
    }
    this->_atlas_model->_actions_to_do.push_back(
        new FillAction(this->_atlas_model, timestamp, this->_schedule->_core));
}
